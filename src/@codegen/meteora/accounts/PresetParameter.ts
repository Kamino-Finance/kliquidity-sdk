...
export class PresetParameter {
  ...
  static readonly layout = borsh.struct<PresetParameter>([
    borsh.u16("binStep"),
  ...
  static async fetch(
    ...
    if (info.programAddress !== programId) {
      throw new Error(
        `PresetParameterFields account ${address} belongs to wrong program ${info.programAddress}, expected ${programId}`
      )
    }
  ...
  static async fetchMultiple(
    ...
    return infos.map((info) => {
      ...
      if (info.programAddress !== programId) {
        throw new Error(
          `PresetParameterFields account ${info.address} belongs to wrong program ${info.programAddress}, expected ${programId}`
        )
      }
