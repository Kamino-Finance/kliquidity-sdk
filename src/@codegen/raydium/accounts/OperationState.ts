...
export class OperationState {
  ...
  static decode(data: Uint8Array): OperationState {
    if (data.length < OperationState.discriminator.length) {
      throw new Error("invalid account discriminator")
    }
    for (let i = 0; i < OperationState.discriminator.length; i++) {
      if (data[i] !== OperationState.discriminator[i]) {
        throw new Error("invalid account discriminator")
      }
    }

    const dec = OperationState.layout.decode(
      data.subarray(OperationState.discriminator.length)
    )
