...
export function getInitializePoolDiscriminatorBytes() { return fixEncoderSize(getBytesEncoder(), 8).encode(INITIALIZE_POOL_DISCRIMINATOR); }
...
Instruction<TProgram> & InstructionWithData<ReadonlyUint8Array> & InstructionWithAccounts<[TAccountWhirlpoolsConfig extends string ? ReadonlyAccount<TAccountWhirlpoolsConfig> : TAccountWhirlpoolsConfig, TAccountTokenMintA extends string ? ReadonlyAccount<TAccountTokenMintA> : TAccountTokenMintA, TAccountTokenMintB extends string ? ReadonlyAccount<TAccountTokenMintB> : TAccountTokenMintB, TAccountFunder extends string ? WritableSignerAccount<TAccountFunder> & AccountSignerMeta<TAccountFunder> : TAccountFunder, TAccountWhirlpool extends string ? WritableAccount<TAccountWhirlpool> : TAccountWhirlpool, TAccountTokenVaultA extends string ? WritableSignerAccount<TAccountTokenVaultA> & AccountSignerMeta<TAccountTokenVaultA> : TAccountTokenVaultA, TAccountTokenVaultB extends string ? WritableSignerAccount<TAccountTokenVaultB> & AccountSignerMeta<TAccountTokenVaultB> : TAccountTokenVaultB, TAccountFeeTier extends string ? ReadonlyAccount<TAccountFeeTier> : TAccountFeeTier, TAccountTokenProgram extends string ? ReadonlyAccount<TAccountTokenProgram> : TAccountTokenProgram, TAccountSystemProgram extends string ? ReadonlyAccount<TAccountSystemProgram> : TAccountSystemProgram, TAccountRent extends string ? ReadonlyAccount<TAccountRent> : TAccountRent, ...TRemainingAccounts]>;

export type InitializePoolInstructionData = { discriminator: ReadonlyUint8Array; whirlpoolBump: number; tickSpacing: number; initialSqrtPrice: bigint;  };

export type InitializePoolInstructionDataArgs = { whirlpoolBump: number; tickSpacing: number; initialSqrtPrice: number | bigint;  };

export function getInitializePoolInstructionDataEncoder(): FixedSizeEncoder<InitializePoolInstructionDataArgs> {
    return transformEncoder(getStructEncoder([['discriminator', fixEncoderSize(getBytesEncoder(), 8)], ['whirlpoolBump', getU8Encoder()], ['tickSpacing', getU16Encoder()], ['initialSqrtPrice', getU128Encoder()]]), (value) => ({ ...value, discriminator: INITIALIZE_POOL_DISCRIMINATOR }));
}

export function getInitializePoolInstructionDataDecoder(): FixedSizeDecoder<InitializePoolInstructionData> {
    return getStructDecoder([['discriminator', fixDecoderSize(getBytesDecoder(), 8)], ['whirlpoolBump', getU8Decoder()], ['tickSpacing', getU16Decoder()], ['initialSqrtPrice', getU128Decoder()]]);
}
