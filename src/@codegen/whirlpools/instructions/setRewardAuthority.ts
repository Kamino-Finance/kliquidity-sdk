...
export function getSetRewardAuthorityDiscriminatorBytes() { return fixEncoderSize(getBytesEncoder(), 8).encode(SET_REWARD_AUTHORITY_DISCRIMINATOR); }
...
export type SetRewardAuthorityInstructionData = { discriminator: ReadonlyUint8Array; rewardIndex: number;  };
...
export function getSetRewardAuthorityInstructionDataEncoder(): FixedSizeEncoder<SetRewardAuthorityInstructionDataArgs> {
    return transformEncoder(getStructEncoder([['discriminator', fixEncoderSize(getBytesEncoder(), 8)], ['rewardIndex', getU8Encoder()]]), (value) => ({ ...value, discriminator: SET_REWARD_AUTHORITY_DISCRIMINATOR }));
}
...
export function getSetRewardAuthorityInstructionDataCodec(): FixedSizeCodec<SetRewardAuthorityInstructionDataArgs, SetRewardAuthorityInstructionData> {
    return combineCodec(getSetRewardAuthorityInstructionDataEncoder(), getSetRewardAuthorityInstructionDataDecoder());
}
...
const getNextAccount = () => {
  ...
  return { programAddress: instruction.programAddress, accounts: { whirlpool: getNextAccount(), rewardAuthority: getNextAccount(), newRewardAuthority: getNextAccount() }, data: getSetRewardAuthorityInstructionDataDecoder().decode(instruction.data) };
}