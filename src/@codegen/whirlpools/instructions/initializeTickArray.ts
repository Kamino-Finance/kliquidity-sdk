...
export const INITIALIZE_TICK_ARRAY_DISCRIMINATOR = new Uint8Array([11, 188, 193, 214, 141, 91, 149, 184]);

export function getInitializeTickArrayDiscriminatorBytes() { return fixEncoderSize(getBytesEncoder(), 8).encode(INITIALIZE_TICK_ARRAY_DISCRIMINATOR); }
...
export type InitializeTickArrayInstructionData = { discriminator: ReadonlyUint8Array; startTickIndex: number;  };
...
export function getInitializeTickArrayInstructionDataEncoder(): FixedSizeEncoder<InitializeTickArrayInstructionDataArgs> {
    return transformEncoder(getStructEncoder([['discriminator', fixEncoderSize(getBytesEncoder(), 8)], ['startTickIndex', getI32Encoder()]]), (value) => ({ ...value, discriminator: INITIALIZE_TICK_ARRAY_DISCRIMINATOR }));
}

export function getInitializeTickArrayInstructionDataDecoder(): FixedSizeDecoder<InitializeTickArrayInstructionData> {
    return getStructDecoder([['discriminator', fixDecoderSize(getBytesDecoder(), 8)], ['startTickIndex', getI32Decoder()]]);
}

export function getInitializeTickArrayInstructionDataCodec(): FixedSizeCodec<InitializeTickArrayInstructionDataArgs, InitializeTickArrayInstructionData> {
    return combineCodec(getInitializeTickArrayInstructionDataEncoder(), getInitializeTickArrayInstructionDataDecoder());
}
...
return Object.freeze({ accounts: [getAccountMeta(accounts.whirlpool), getAccountMeta(accounts.funder), getAccountMeta(accounts.tickArray), getAccountMeta(accounts.systemProgram)], data: getInitializeTickArrayInstructionDataEncoder().encode(args as InitializeTickArrayInstructionDataArgs), programAddress } as InitializeTickArrayInstruction<TProgramAddress, TAccountWhirlpool, TAccountFunder, TAccountTickArray, TAccountSystemProgram>);
}

export type ParsedInitializeTickArrayInstruction<TProgram extends string = typeof WHIRLPOOL_PROGRAM_ADDRESS, TAccountMetas extends readonly AccountMeta[] = readonly AccountMeta[]> = { programAddress: Address<TProgram>;
accounts: {
...
export function parseInitializeTickArrayInstruction<TProgram extends string, TAccountMetas extends readonly AccountMeta[]>(instruction: Instruction<TProgram> & InstructionWithAccounts<TAccountMetas> & InstructionWithData<ReadonlyUint8Array>): ParsedInitializeTickArrayInstruction<TProgram, TAccountMetas> {
...
const getNextAccount = () => {
  ...
  return { programAddress: instruction.programAddress, accounts: { whirlpool: getNextAccount(), funder: getNextAccount(), tickArray: getNextAccount(), systemProgram: getNextAccount() }, data: getInitializeTickArrayInstructionDataDecoder().decode(instruction.data) };
}