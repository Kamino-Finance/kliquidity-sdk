...
export function getCloseBundledPositionDiscriminatorBytes() { return fixEncoderSize(getBytesEncoder(), 8).encode(CLOSE_BUNDLED_POSITION_DISCRIMINATOR); }
...
export function getCloseBundledPositionInstructionDataEncoder(): FixedSizeEncoder<CloseBundledPositionInstructionDataArgs> {
    return transformEncoder(getStructEncoder([['discriminator', fixEncoderSize(getBytesEncoder(), 8)], ['bundleIndex', getU16Encoder()]]), (value) => ({ ...value, discriminator: CLOSE_BUNDLED_POSITION_DISCRIMINATOR }));
}

export function getCloseBundledPositionInstructionDataDecoder(): FixedSizeDecoder<CloseBundledPositionInstructionData> {
    return getStructDecoder([['discriminator', fixDecoderSize(getBytesDecoder(), 8)], ['bundleIndex', getU16Decoder()]]);
}

export function getCloseBundledPositionInstructionDataCodec(): FixedSizeCodec<CloseBundledPositionInstructionDataArgs, CloseBundledPositionInstructionData> {
    return combineCodec(getCloseBundledPositionInstructionDataEncoder(), getCloseBundledPositionInstructionDataDecoder());
}

export type CloseBundledPositionInput<TAccountBundledPosition extends string = string, TAccountPositionBundle extends string = string, TAccountPositionBundleTokenAccount extends string = string, TAccountPositionBundleAuthority extends string = string, TAccountReceiver extends string = string> =  {
...
const getNextAccount = () => {
  ...
  return { programAddress: instruction.programAddress, accounts: { bundledPosition: getNextAccount(), positionBundle: getNextAccount(), positionBundleTokenAccount: getNextAccount(), positionBundleAuthority: getNextAccount(), receiver: getNextAccount() }, data: getCloseBundledPositionInstructionDataDecoder().decode(instruction.data) };
}