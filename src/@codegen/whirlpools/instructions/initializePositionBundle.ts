...
export function getInitializePositionBundleInstructionDataEncoder(): FixedSizeEncoder<InitializePositionBundleInstructionDataArgs> {
    return transformEncoder(getStructEncoder([['discriminator', fixEncoderSize(getBytesEncoder(), 8)]]), (value) => ({ ...value, discriminator: INITIALIZE_POSITION_BUNDLE_DISCRIMINATOR }));
}
...
export function getInitializePositionBundleInstructionDataCodec(): FixedSizeCodec<InitializePositionBundleInstructionDataArgs, InitializePositionBundleInstructionData> {
    return combineCodec(getInitializePositionBundleInstructionDataEncoder(), getInitializePositionBundleInstructionDataDecoder());
}

export type InitializePositionBundleInput<TAccountPositionBundle extends string = string, TAccountPositionBundleMint extends string = string, TAccountPositionBundleTokenAccount extends string = string, TAccountPositionBundleOwner extends string = string, TAccountFunder extends string = string, TAccountTokenProgram extends string = string, TAccountSystemProgram extends string = string, TAccountRent extends string = string, TAccountAssociatedTokenProgram extends string = string> =  {
...
const getNextAccount = () => {
  ...
  return { programAddress: instruction.programAddress, accounts: { positionBundle: getNextAccount(), positionBundleMint: getNextAccount(), positionBundleTokenAccount: getNextAccount(), positionBundleOwner: getNextAccount(), funder: getNextAccount(), tokenProgram: getNextAccount(), systemProgram: getNextAccount(), rent: getNextAccount(), associatedTokenProgram: getNextAccount() }, data: getInitializePositionBundleInstructionDataDecoder().decode(instruction.data) };
}