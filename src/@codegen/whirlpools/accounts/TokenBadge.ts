...
export const TOKEN_BADGE_DISCRIMINATOR = new Uint8Array([116, 219, 204, 229, 249, 116, 255, 150]);

export function getTokenBadgeDiscriminatorBytes() { return fixEncoderSize(getBytesEncoder(), 8).encode(TOKEN_BADGE_DISCRIMINATOR); }

export type TokenBadge = { discriminator: ReadonlyUint8Array; whirlpoolsConfig: Address; tokenMint: Address;  };
...
export function getTokenBadgeEncoder(): FixedSizeEncoder<TokenBadgeArgs> {
    return transformEncoder(getStructEncoder([['discriminator', fixEncoderSize(getBytesEncoder(), 8)], ['whirlpoolsConfig', getAddressEncoder()], ['tokenMint', getAddressEncoder()]]), (value) => ({ ...value, discriminator: TOKEN_BADGE_DISCRIMINATOR }));
}

/** Gets the decoder for {@link TokenBadge} account data. */
export function getTokenBadgeDecoder(): FixedSizeDecoder<TokenBadge> {
    return getStructDecoder([['discriminator', fixDecoderSize(getBytesDecoder(), 8)], ['whirlpoolsConfig', getAddressDecoder()], ['tokenMint', getAddressDecoder()]]);
}

/** Gets the codec for {@link TokenBadge} account data. */
export function getTokenBadgeCodec(): FixedSizeCodec<TokenBadgeArgs, TokenBadge> {
    return combineCodec(getTokenBadgeEncoder(), getTokenBadgeDecoder());
}

export function decodeTokenBadge<TAddress extends string = string>(encodedAccount: EncodedAccount<TAddress>): Account<TokenBadge, TAddress>;
export function decodeTokenBadge<TAddress extends string = string>(encodedAccount: MaybeEncodedAccount<TAddress>): MaybeAccount<TokenBadge, TAddress>;
export function decodeTokenBadge<TAddress extends string = string>(encodedAccount: EncodedAccount<TAddress> | MaybeEncodedAccount<TAddress>): Account<TokenBadge, TAddress> | MaybeAccount<TokenBadge, TAddress> {
  return decodeAccount(encodedAccount as MaybeEncodedAccount<TAddress>, getTokenBadgeDecoder());
}
