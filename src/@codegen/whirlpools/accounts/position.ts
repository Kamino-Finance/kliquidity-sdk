...
import { assertAccountExists, assertAccountsExist, combineCodec, decodeAccount, fetchEncodedAccount, fetchEncodedAccounts, fixDecoderSize, fixEncoderSize, getAddressDecoder, getAddressEncoder, getArrayDecoder, getArrayEncoder, getBytesDecoder, getBytesEncoder, getI32Decoder, getI32Encoder, getStructDecoder, getStructEncoder, getU128Decoder, getU128Encoder, getU64Decoder, getU64Encoder, transformEncoder, type Account, type Address, type EncodedAccount, type FetchAccountConfig, type FetchAccountsConfig, type FixedSizeCodec, type FixedSizeDecoder, type FixedSizeEncoder, type MaybeAccount, type MaybeEncodedAccount, type ReadonlyUint8Array } from '@solana/kit';
...
export type Position = { discriminator: ReadonlyUint8Array; whirlpool: Address; positionMint: Address; liquidity: bigint; tickLowerIndex: number; tickUpperIndex: number; feeGrowthCheckpointA: bigint; feeOwedA: bigint; feeGrowthCheckpointB: bigint; feeOwedB: bigint; rewardInfos: Array<PositionRewardInfo>;  };

export type PositionArgs = { whirlpool: Address; positionMint: Address; liquidity: number | bigint; tickLowerIndex: number; tickUpperIndex: number; feeGrowthCheckpointA: number | bigint; feeOwedA: number | bigint; feeGrowthCheckpointB: number | bigint; feeOwedB: number | bigint; rewardInfos: Array<PositionRewardInfoArgs>;  };

/** Gets the encoder for {@link PositionArgs} account data. */
export function getPositionEncoder(): FixedSizeEncoder<PositionArgs> {
    return transformEncoder(getStructEncoder([['discriminator', fixEncoderSize(getBytesEncoder(), 8)], ['whirlpool', getAddressEncoder()], ['positionMint', getAddressEncoder()], ['liquidity', getU128Encoder()], ['tickLowerIndex', getI32Encoder()], ['tickUpperIndex', getI32Encoder()], ['feeGrowthCheckpointA', getU128Encoder()], ['feeOwedA', getU64Encoder()], ['feeGrowthCheckpointB', getU128Encoder()], ['feeOwedB', getU64Encoder()], ['rewardInfos', getArrayEncoder(getPositionRewardInfoEncoder(), { size: 3 })]]), (value) => ({ ...value, discriminator: POSITION_DISCRIMINATOR }));
}

/** Gets the decoder for {@link Position} account data. */
export function getPositionDecoder(): FixedSizeDecoder<Position> {
    return getStructDecoder([['discriminator', fixDecoderSize(getBytesDecoder(), 8)], ['whirlpool', getAddressDecoder()], ['positionMint', getAddressDecoder()], ['liquidity', getU128Decoder()], ['tickLowerIndex', getI32Decoder()], ['tickUpperIndex', getI32Decoder()], ['feeGrowthCheckpointA', getU128Decoder()], ['feeOwedA', getU64Decoder()], ['feeGrowthCheckpointB', getU128Decoder()], ['feeOwedB', getU64Decoder()], ['rewardInfos', getArrayDecoder(getPositionRewardInfoDecoder(), { size: 3 })]]);
}
