...
export const FEE_TIER_DISCRIMINATOR = new Uint8Array([56, 75, 159, 76, 142, 68, 190, 105]);

export function getFeeTierDiscriminatorBytes() { return fixEncoderSize(getBytesEncoder(), 8).encode(FEE_TIER_DISCRIMINATOR); }

export type FeeTier = { discriminator: ReadonlyUint8Array; whirlpoolsConfig: Address; tickSpacing: number; defaultFeeRate: number;  };

export type FeeTierArgs = { whirlpoolsConfig: Address; tickSpacing: number; defaultFeeRate: number;  };

/** Gets the encoder for {@link FeeTierArgs} account data. */
export function getFeeTierEncoder(): FixedSizeEncoder<FeeTierArgs> {
    return transformEncoder(getStructEncoder([['discriminator', fixEncoderSize(getBytesEncoder(), 8)], ['whirlpoolsConfig', getAddressEncoder()], ['tickSpacing', getU16Encoder()], ['defaultFeeRate', getU16Encoder()]]), (value) => ({ ...value, discriminator: FEE_TIER_DISCRIMINATOR }));
}

/** Gets the decoder for {@link FeeTier} account data. */
export function getFeeTierDecoder(): FixedSizeDecoder<FeeTier> {
    return getStructDecoder([['discriminator', fixDecoderSize(getBytesDecoder(), 8)], ['whirlpoolsConfig', getAddressDecoder()], ['tickSpacing', getU16Decoder()], ['defaultFeeRate', getU16Decoder()]]);
}
...
export function decodeFeeTier<TAddress extends string = string>(encodedAccount: EncodedAccount<TAddress>): Account<FeeTier, TAddress>;
export function decodeFeeTier<TAddress extends string = string>(encodedAccount: MaybeEncodedAccount<TAddress>): MaybeAccount<FeeTier, TAddress>;
export function decodeFeeTier<TAddress extends string = string>(encodedAccount: EncodedAccount<TAddress> | MaybeEncodedAccount<TAddress>): Account<FeeTier, TAddress> | MaybeAccount<FeeTier, TAddress> {
  return decodeAccount(encodedAccount as MaybeEncodedAccount<TAddress>, getFeeTierDecoder());
}
