...
export class Kamino {
  ...
  getAllScopePriceFeedsForStrategy = (
    ...
    if (strategy.reward0Vault !== DEFAULT_PUBLIC_KEY && strategy.reward0Vault !== strategy.baseVaultAuthority) {
      res.rewardsFeeds[0] = collateralInfos[Number(strategy.reward0CollateralId)].scopeFeed;
    }
    if (strategy.reward1Vault !== DEFAULT_PUBLIC_KEY && strategy.reward1Vault !== strategy.baseVaultAuthority) {
      res.rewardsFeeds[1] = collateralInfos[Number(strategy.reward1CollateralId)].scopeFeed;
    }
    if (strategy.reward2Vault !== DEFAULT_PUBLIC_KEY && strategy.reward2Vault !== strategy.baseVaultAuthority) {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      if (rewardInfo.rewardMint !== DEFAULT_PUBLIC_KEY && Number(rewardInfo.decimals) > 0) {
  ...
  legacyGetStrategiesShareData = async (
    ...
    const collInfos = collateralInfos ? collateralInfos : await this.getCollateralInfos();
    const allScopePrices = strategiesWithAddresses.flatMap((x) =>
      this.strategyTokenScopeFeedsToArray(this.getAllScopePriceFeedsForStrategy(x.strategy, collInfos))
    );
  ...
  private getBalance = <PoolT, PositionT>(
    ...
    fetchBalance: (
      ...
      position: PositionT,
      collateralInfos: CollateralInfo[],
      prices?: Record<Address, OraclePrices>,
      disabledTokensPrices?: Map<Address, Decimal>
   ...
   */
  getWhirlpoolByAddress = async (whirlpool: Address) => unwrapAccount(await fetchMaybeWhirlpool(this._rpc, whirlpool));
  ...
  getRaydiumAmmConfig = async (config: Address) => unwrapAccount(await fetchMaybeAmmConfig(this._rpc, config));

  /**
   * Get Raydium pool from public key
   * @param pool pubkey of the orca whirlpool
   */
  getRaydiumPoolByAddress = async (pool: Address) => unwrapAccount(await fetchMaybePoolState(this._rpc, pool));
  ...
  withdrawShares = async (
    ...
    if (isRaydium) {
      const raydiumPosition = unwrapAccount(
        await fetchMaybePersonalPositionState(this._rpc, strategyState.strategy.position)
      );
      if (!raydiumPosition) {
  ...
  private getGlobalConfigValue = (value: Address | bigint | boolean): Uint8Array => {
  ...
  updateGlobalConfig = (
    ...
    let updateConfigIx = getUpdateGlobalConfigInstruction(
      {
        adminAuthority: owner,
        globalConfig,
        systemProgram: SYSTEM_PROGRAM_ADDRESS,
    ...
    if (
      (globalConfigOption === GlobalConfigOption.UpdateScopePriceId ||
        globalConfigOption === GlobalConfigOption.AddScopePriceId) &&
      typeof value === 'string' &&
      isAddress(value)
    ) {
      updateConfigIx = appendAccounts(updateConfigIx, [{ address: value, role: AccountRole.READONLY }]);
    }
  ...
  private getSingleSidedDepositIxs = async (
    ...
    const singleSidedDepositIx = getSingleTokenDepositWithMinInstruction(
      { ...args, ...accounts },
      { programAddress: this.getProgramID() }
    );
  ...
  withdrawTopupVault = async (owner: TransactionSigner, amount: Decimal): Promise<Instruction> => {
    ...
    const withdrawIxn = getWithdrawFromTopupInstruction(
      { ...args, ...accounts },
      { programAddress: this.getProgramID() }
    );
    return withdrawIxn;
  ...
  executiveWithdraw = async (
    ...
    let executiveWithdrawIx = getExecutiveWithdrawInstruction(
      { ...args, ...accounts },
      { programAddress: this.getProgramID() }
    );
    ...
    if (Number(strategyState.strategyDex) === dexToNumber('ORCA')) {
      ...
      for (let i = 0; i < 3; i++) {
        if (whirlpool.rewardInfos[i].mint !== DEFAULT_PUBLIC_KEY) {
          ...
          const ix = getUpdateRewardMappingInstruction(
            { ...args, ...accounts },
            { programAddress: this.getProgramID() }
          );
